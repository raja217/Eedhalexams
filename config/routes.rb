ActionController::Routing::Routes.draw do |map|
  ...
  map.resources :batches, :collection=>{:batches_ajax=>[:get]} do |batch|
    batch.resources :exam_groups do |ex|
      ex.resources :modules do |m|
        m.resources :questions
      end
    end
  end

  map.resources :exam_groups do |m|
    m.resources :answers, :collection => {:sheet => [:get,:post], :exam => [:get,:post],:answer => [:get,:post], :final => [:get,:post], :result => [:get,:post]}
  end
