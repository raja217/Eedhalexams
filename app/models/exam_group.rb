class ExamGroup < ActiveRecord::Base
  ...
  has_many :student_additional_fields
  has_many :questions
  has_many :exams
  ...
  accepts_nested_attributes_for :student_additional_fields
  ...
  def total_marks(student)
    marks = StudentAdditionalField.find(:all)
    total_marks = 0
    max_total = 0
    marks.each do |marks|
      #exam_score = StudentAdditionalField.sum
      total_marks = total_marks + (exam_score.marks || 0) unless exam_score.nil?
      max_total = max_total + marks.marks unless exam_score.nil?
    end
