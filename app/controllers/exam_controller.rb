class ExamController < ApplicationController
  ...
  def modules
    ...
    if request.post? and @additional_field.save
      ...
      redirect_to :controller => "exam", :action => "modules"
    end
  end
  ...
  def generated_report
    ...
    if params[:student].nil?
      ...
      #@answers = Answer.find(:all, :conditions => ['user_id = ? ',@student.user_id])
     # @total = Answer.sum(:marks, :conditions => ['user_id = ?',@student.user_id])
      @exams = []
      @modules.each do |mod|
        #exam = ExamGroup.find_by_id_and_student_additional_field_id(@exam_group.id,sub.id)
  ...
  def consolidated_exam_report
    ...
    filename = "Report-#{@exam_group.name}" + ".xls"
