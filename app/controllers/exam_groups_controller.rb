class ExamGroupsController < ApplicationController
  ...
  def new
    ...
    @batch = Batch.find_by_id(params[:batch_id])
    @course = Course.find_by_id(params[:batch_id])
  ...
  def create
    ...
    session[:ques] = []
    @modules = StudentAdditionalField.find(:all, :conditions => ['id in (?)',params[:modules]])
    @modules.each do |q|
      session[:ques].push q.id
    end
    mod = session[:ques].join(',')
    @exam_group.modules = mod
    if @exam_group.save
  ...
  def show
    @exam_group = ExamGroup.find_by_id(params[:id])
